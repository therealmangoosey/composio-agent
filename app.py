...
ROOT=Path(__file__).resolve().parent; DATA=ROOT/"data"; CONFIG_FILE=DATA/"config.json"; MEMORY_FILE=DATA/"memory.json"; ERROR_FILE=DATA/"errors.log"; ENV_FILE=ROOT/".env"; MAX_AGENTS=4
PROVIDERS={"GROQ":("https://api.groq.com/openai/v1",["openai/gpt-oss-20b","openai/gpt-oss-120b"],True),"Google Gemini":("https://generativelanguage.googleapis.com/v1beta/openai",["gemini-2.5-flash"],True),"OpenAI":("https://api.openai.com/v1",["gpt-4o-mini"],False),"OpenRouter":("https://openrouter.ai/api/v1",["openrouter/free"],True),"Cerebras":("https://api.cerebras.ai/v1",["gpt-oss-120b"],True),"DeepSeek":("https://api.deepseek.com",["deepseek-chat"],False),"xAI Grok":("https://api.x.ai/v1",["grok-4.1-mini"],False)}
ENV_KEYS={"GROQ":"GROQ_API_KEYS","Google Gemini":"GEMINI_API_KEYS","OpenAI":"OPENAI_API_KEYS","OpenRouter":"OPENROUTER_API_KEYS","Cerebras":"CEREBRAS_API_KEYS","DeepSeek":"DEEPSEEK_API_KEYS","xAI Grok":"XAI_API_KEYS"}
DEFAULT_SYSTEM=("You are Tab Assistant. Be concise and useful. You have access to Composio through the application's approved tool-execution path. "
...
def save_dotenv(values):
    ...
    old.update({k:str(v) for k,v in values.items() if v is not None}); ENV_FILE.write_text("# Secrets; do not commit.\n"+"\n".join(f"{k}={v}" for k,v in sorted(old.items()) if v)+"\n",encoding="utf-8")
    try: os.chmod(ENV_FILE,0o600)
    except OSError: pass

def defaults():
    return {"providers":{k:{"base_url":v[0],"models":list(v[1]),"keys":[]} for k,v in PROVIDERS.items()},"selected_provider":"GROQ","selected_model":PROVIDERS["GROQ"][1][0],"mode":"free","composio":{"api_key":"","user_id":"tab-owner","toolkit_version":"latest"},"discord":{"token":"","channel_ids":[],"allowed_user_ids":[],"autostart":True},"settings":{"system_prompt":DEFAULT_SYSTEM,"max_history":8,"temperature":0.7,"max_research_agents":4}}
...
def load_config():
    ...
    for name,info in cfg["providers"].items():
        ...
        if raw:
            old=info["keys"]; flags=[bool(x.get("free",PROVIDERS[name][2])) for x in old]; info["keys"]=[{"key":k,"free":flags[i] if i<len(flags) else PROVIDERS[name][2]} for i,k in enumerate(x.strip() for x in raw.split(",") if x.strip())]
    cfg["composio"]["api_key"]=os.getenv("COMPOSIO_API_KEY",""); cfg["discord"]["token"]=os.getenv("DISCORD_BOT_TOKEN",""); legacy=os.getenv("DISCORD_ALLOWED_CHANNEL_ID","")
    if legacy and not cfg["discord"].get("channel_ids"): cfg["discord"]["channel_ids"]=[x.strip() for x in legacy.split(",") if x.strip()]
    cfg["settings"].setdefault("max_research_agents",4); cfg["settings"].setdefault("system_prompt",DEFAULT_SYSTEM); return cfg

def save_config(cfg):
    DATA.mkdir(parents=True,exist_ok=True); env={ENV_KEYS[n]:",".join(x["key"] for x in i["keys"]) for n,i in cfg["providers"].items()}; env.update({"COMPOSIO_API_KEY":cfg["composio"].get("api_key",""),"DISCORD_BOT_TOKEN":cfg["discord"].get("token",""),"DISCORD_ALLOWED_CHANNEL_ID":",".join(cfg["discord"].get("channel_ids",[]))}); save_dotenv(env); public=copy.deepcopy(cfg)
...
def fetch_models(cfg,provider,key):
    try:
        data=requests.get(cfg["providers"][provider]["base_url"].rstrip("/")+"/models",headers={"Authorization":f"Bearer {key}"},timeout=8).json().get("data",[]); models=[x["id"] for x in data if x.get("id")]
...
def send(cfg,session,system=None,temperature=None,json_mode=False):
    ...
    order.sort(key=lambda x:x[0]); messages=[{"role":"system","content":system or cfg["settings"]["system_prompt"]}]+session["messages"][-int(cfg["settings"].get("max_history",8)):]; last_error=None
...
def needs_external_research(text):
    t=text.lower().strip()
    explicit=bool(re.match(r"^/?research\b",t))
    web_words=("search","look up","find","where can i","places","shops","companies","businesses","print place","printing","latest","current","today","near me","in southampton","in the uk","every ","all ")
    compare=any(k in t for k in ("compare","research","look into","investigate"))
    return explicit or compare or any(k in t for k in web_words)

def is_research(text): return needs_external_research(text)
def research_plan(topics): return "🔎 **Research plan**\n"+"\n".join(f"{i}. {t}" for i,t in enumerate(topics,1))+f"\n\nThis will run up to {len(topics)} research agent(s) in parallel. **Nothing will run until you approve.**"

def research_agents(cfg,topics):
    limit=max(1,min(MAX_AGENTS,int(cfg["settings"].get("max_research_agents",MAX_AGENTS)),len(topics)))
    def worker(topic):
        s=new_session(); add(s,"user",f"Research this topic using available external tools. Topic: {topic}. Return concise factual findings, names, addresses/contact details when relevant, and source URLs. If no external search tool is actually available, say so instead of inventing results."); return send(cfg,s,system="You are a research worker. You may use the application's approved Composio/search tools only when the application actually supplies and executes them. Never emit fake tool-call XML or claim a search happened unless a tool result confirms it.",temperature=0.2)
    out=[]
...
def synthesize(cfg,results):
    s=new_session(); add(s,"user","Synthesize these reports. Preserve topic boundaries, flag uncertainty and never invent citations.\n\n"+"\n\n".join(f"TOPIC: {t}\n{r}" for t,r in results)); return send(cfg,s,system="You are the final research synthesizer. Be accurate. Do not invent sources, URLs, places, prices, or claims.",temperature=0.2)
...
def allowed_user(cfg,user_id):
    allowed=cfg["discord"].get("allowed_user_ids",[]); return not allowed or str(user_id) in {str(x) for x in allowed}
def channel_enabled(cfg,channel_id): return str(channel_id) in {str(x) for x in cfg["discord"].get("channel_ids",[])}
...
def start_discord(cfg):
    ...
    @tree.command(name="research",description="Create a research plan requiring approval")
    async def research_cmd(i,topics:str):
        ...
        if not topics_list:await i.response.send_message("Give me something to research.",ephemeral=True);return
    ...
    async def help_cmd(i):await i.response.send_message("/this-channel /add-channel /remove-channel /research /status /invite\nNormal messages work in enabled channels. External research and Composio actions require approval.",ephemeral=True)
    @bot.event
    async def on_ready():
        try:
            for guild in bot.guilds:tree.copy_global_to(guild=guild);await tree.sync(guild=guild)
    ...
    async def on_message(message):
        ...
        try:
            if needs_external_research(text):
                topics=split_topics(text) or [text]
                await message.reply(research_plan(topics),view=approval_view(discord,cfg,"research",topics,message.author.id),mention_author=False);return
            if any(k in text.lower() for k in ("composio","gmail","send an email","save a note","add to list","create a calendar","send a message")):
                plan=composio_plan(cfg,text);preview="\n".join(f"{i}. {x.get('description',x.get('tool','unknown'))}" for i,x in enumerate(plan.get("steps",[]),1));await message.reply("🛠️ **Composio plan**\n"+preview+"\n\nNothing will run until approved.",view=approval_view(discord,cfg,"composio",plan),mention_author=False);return
...
def console_chat(cfg):
    ...
    while True:
        ...
        if text=="/help":p("Research/search requests show an approval prompt. /new resets chat. /quit returns to menu.");continue
        if needs_external_research(text):
            topics=split_topics(text) or [text];p(research_plan(topics))
...
def menu(cfg):
    while True:
        ...
        if c=="1":console_chat(cfg)
            ...
            try:cfg["settings"]["max_research_agents"]=max(1,min(MAX_AGENTS,int(input("Max research agents [1-4]: "))));persist(cfg)
            except ValueError:pass
        elif c=="10":
            link,error=invite_link(cfg);p(link or "❌ "+error)
        elif c=="6":
            try:
                mem=json.loads(MEMORY_FILE.read_text(encoding="utf-8")) if MEMORY_FILE.exists() else {"sessions":[]};p("\n".join(f"{i+1}. {x.get('created','?')} ({len(x.get('messages',[]))} messages)" for i,x in enumerate(mem.get('sessions',[])[-10:])))
